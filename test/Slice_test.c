...
int test_slice_bfloat16()
{
    ...
    BENCH_START(Slice_bfloat16_axis0);
    Slice_bfloat16(node);
    BENCH_END(Slice_bfloat16_axis0);
    ...
    BENCH_START(Slice_bfloat16_rvv_axis0);
    Slice_bfloat16_rvv(node);
    BENCH_END(Slice_bfloat16_rvv_axis0);
    ...
    node->priv = GenerateSliceParam(1, axes + 1, start + 1, end + 1, step + 1);
    memset(node->outputs[0]->datas, 0, sizeof(bfloat16_t) * node->outputs[0]->ndata);
    BENCH_START(Slice_bfloat16_axis1);
    Slice_bfloat16(node);
    BENCH_END(Slice_bfloat16_axis1);
    memcpy(golden, node->outputs[0]->datas, node->outputs[0]->ndata * sizeof(bfloat16_t));

    // rvv optimization test with axis 1
    memset(node->outputs[0]->datas, 0, node->outputs[0]->ndata * sizeof(bfloat16_t));
    BENCH_START(Slice_bfloat16_rvv_axis1);
    Slice_bfloat16_rvv(node);
    BENCH_END(Slice_bfloat16_rvv_axis1);
    ...
    node->priv = GenerateSliceParam(2, axes, start, end, step);
    memset(node->outputs[0]->datas, 0, sizeof(bfloat16_t) * node->outputs[0]->ndata);
    BENCH_START(Slice_bfloat16_bothaxes);
    Slice_bfloat16(node);
    BENCH_END(Slice_bfloat16_bothaxes);
    memcpy(golden, node->outputs[0]->datas, node->outputs[0]->ndata * sizeof(bfloat16_t));

    // rvv optimization test with both axis
    memset(node->outputs[0]->datas, 0, node->outputs[0]->ndata * sizeof(bfloat16_t));
    BENCH_START(Slice_bfloat16_rvv_bothaxes);
    Slice_bfloat16_rvv(node);
    BENCH_END(Slice_bfloat16_rvv_bothaxes);
