...
void Rsqrt_bfloat16(struct onnx_node_t *n)
{
    struct onnx_tensor_t *x = n->inputs[0];
    struct onnx_tensor_t *y = n->outputs[0];
    ...
    for (size_t i = 0, l = y->ndata; i < l; i++) {
        py[i] = (bfloat16_t)1.0 / (bfloat16_t)sqrtf((float32_t)px[i]);
    }
}

void Rsqrt_bfloat16_rvv(struct onnx_node_t *n)
{
    struct onnx_tensor_t *x = n->inputs[0];
    struct onnx_tensor_t *y = n->outputs[0];
    ...
    for (; (vl = __riscv_vsetvl_e16m8(vblkCnt)) > 0; vblkCnt -= vl) {
        ...
        px += vl;
        vy = __riscv_xl_vfsqrt_v_bf16m8(vx, vl);
        // vy = __riscv_xl_vfrec7_v_bf16m8(vy, vl);
        vy = __riscv_xl_vfrdiv_vf_bf16m8(vy, 1, vl);
