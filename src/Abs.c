...
void Abs_bfloat16(struct onnx_node_t *n)
{
    struct onnx_tensor_t *x = n->inputs[0];
    struct onnx_tensor_t *y = n->outputs[0];
    ...
    for (size_t i = 0, l = y->ndata; i < l; i++) {
        py[i] = (bfloat16_t)fabsf((float32_t)px[i]);
    }
}

void Abs_bfloat16_rvv(struct onnx_node_t *n)
{
    struct onnx_tensor_t *x = n->inputs[0];
    struct onnx_tensor_t *y = n->outputs[0];
    bfloat16_t *py = (bfloat16_t *)y->datas;
    ...
    for (; (vl = __riscv_vsetvl_e16m8(blkCnt)) > 0; blkCnt -= vl) {
        ...
        px += vl;
        vx = __riscv_xl_vfsgnjx_vv_bf16m8(vx, vx, vl);
